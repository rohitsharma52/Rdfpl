...
const dryFruits = [
    'url(/front/assets/png/Almonds.png)', // replace with actual image URLs
    'url(/front/assets/png/2.png)',
    'url(/front/assets/png/3.png)',
    'url(/front/assets/png/4.png)',
];

function createDryFruit() {
    const dryFruit = document.createElement('div');
    dryFruit.classList.add('dry-fruit');
    dryFruit.style.backgroundImage = dryFruits[Math.floor(Math.random() * dryFruits.length)];
    dryFruit.style.left = Math.random() * 100 + 'vw';
    dryFruit.style.animationDuration = Math.random() * 2 + 3 + 's';
    rainContainer.appendChild(dryFruit);

    setTimeout(() => {
        dryFruit.remove();
    }, 5000); // Remove the element after it has fallen
}

setInterval(createDryFruit, 200); // Create a new dry fruit every 200ms
  ...
  $(function () { // wait for document ready
    ...
    var wipeAnimation = new TimelineMax()
      .fromTo("section.panel.blue", 1, {x: "100%"}, {x: "0%", ease: Linear.easeNone}) // Blue section in from right
      .fromTo("section.panel.turqoise", 1, {x: "-100%"}, {x: "0%", ease: Linear.easeNone}) // Turquoise section in from left
      .fromTo("section.panel.green", 1, {y: "100%"}, {y: "0%", ease: Linear.easeNone}) // Green section in from bottom
      .fromTo("section.panel.bordeaux", 1, {y: "-100%"}, {y: "0%", ease: Linear.easeNone}); // Bordeaux section in from top

    // create scene to pin and link animation
    new ScrollMagic.Scene({
        triggerElement: "#pinContainer",
        triggerHook: "onLeave",
        duration: "400%" // Adjusted to fit all panels
      })
      .setPin("#pinContainer")
